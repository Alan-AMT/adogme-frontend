...
function emptyVac(): Vaccination {
  return { id: _nextId++, nombre: '', fecha: '', proximaDosis: '', verificada: false }
}
...
export function MedicalStep({ formData, errors, update }: Props) {
  ...
  function removeVaccine(id: number) {
    update('vacunas', formData.vacunas.filter(v => v.id !== id))
  }

  function updateNewVac<K extends keyof Vaccination>(key: K, val: Vaccination[K]) {
      ...
      <div className="sv-form-section">
        ...
        <div className="sv-form-section__body">
          ...
          {formData.vacunas.length > 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                <thead>
                  <tr style={{ background: '#fafafa', borderBottom: '1px solid #f4f4f5' }}>
                    {['Vacuna', 'Fecha', 'Próx. dosis', 'Verificada', ''].map(h => (
                      <th key={h} style={{ padding: '0.55rem 0.85rem', fontWeight: 900, color: '#a1a1aa', textAlign: 'left', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {formData.vacunas.map(vac => (
                    <tr key={vac.id} style={{ borderBottom: '1px solid #f9fafb' }}>
                      <td style={{ padding: '0.65rem 0.85rem', fontWeight: 700, color: '#18181b' }}>{vac.nombre}</td>
                      <td style={{ padding: '0.65rem 0.85rem', color: '#52525b' }}>{vac.fecha}</td>
                      <td style={{ padding: '0.65rem 0.85rem', color: '#71717a' }}>{vac.proximaDosis || '—'}</td>
                      <td style={{ padding: '0.65rem 0.85rem' }}>
                        <span style={{
                          display: 'inline-flex', alignItems: 'center', gap: '0.2rem',
                          fontSize: '0.72rem', fontWeight: 900, borderRadius: 999,
                      ...
                      <td style={{ padding: '0.65rem 0.85rem' }}>
                        <button
                          ...
                          style={{ background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '0.5rem', width: 28, height: 28, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        >
            ...
            <p style={{ fontSize: '0.82rem', color: '#a1a1aa', fontWeight: 500, marginBottom: '1rem' }}>
          ...
          <div style={{ background: '#f9fafb', borderRadius: '0.9rem', padding: '1rem', border: '1.5px dashed #e4e4e7' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem', marginBottom: '0.65rem' }}>
              ...
              <div className="sv-field">
                ...
                <input
                  type="date" className="sv-field__input"
                  value={newVac.fecha}
                  onChange={e => updateNewVac('fecha', e.target.value)}
                />
              </div>
              <div className="sv-field">
                <label className="sv-field__label">Próxima dosis (opcional)</label>
                <input
                  type="date" className="sv-field__input"
                  value={newVac.proximaDosis ?? ''}
                  onChange={e => updateNewVac('proximaDosis', e.target.value || undefined)}
                />
              </div>
              <div className="sv-field" style={{ justifyContent: 'flex-end' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', paddingTop: '1.6rem' }}>
                  <input
                    ...
                    onChange={e => updateNewVac('verificada', e.target.checked)}
                    style={{ width: 16, height: 16, cursor: 'pointer' }}
                  />
                  <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#374151' }}>Verificada por veterinario</span>
                </label>
              </div>
            </div>
            <button
              ...
              style={{
                ...
                border: 'none', fontSize: '0.82rem', fontWeight: 900,
                cursor: (!newVac.nombre.trim() || !newVac.fecha) ? 'not-allowed' : 'pointer',
