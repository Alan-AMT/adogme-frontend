...
const DIMENSION_TO_CATEGORY: QuizCategory[] = ['activity', 'housing', 'experience', 'care']
...
function buildResumen(topScore: number): string {
  if (topScore >= 80) return 'Excelente compatibilidad — estos perros se adaptan muy bien a tu perfil'
  if (topScore >= 60) return 'Buena compatibilidad — encontramos candidatos con mucho potencial'
  if (topScore >= 40) return 'Algunos perros que podrían adaptarse a tu estilo de vida'
...
export class MLService implements IMLService {
  ...
  async generateRecommendations(
    ...
    // Persistir en applicants-ms — no bloqueante (endpoint pendiente en BE)
    profileService.updateUserVector(adoptanteId, userVector).catch(err => {
