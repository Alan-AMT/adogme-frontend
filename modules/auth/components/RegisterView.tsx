...
const ROLES: { id: RegisterRole; icon: string; label: string; desc: string }[] = [
  { id: 'adoptante', icon: 'person', label: 'Adoptante', desc: 'Quiero adoptar' },
  { id: 'refugio', icon: 'home_work', label: 'Refugio', desc: 'Tengo un refugio' },
]
...
function AdoptanteSuccessScreen({ email }: { email: string }) {
  return (
    <div className="flex flex-col items-center text-center gap-4 py-4">
      ...
      <div>
        <h2 className="text-[22px] font-[950] text-[#111827] tracking-tight mb-2">¡Verifica tu correo!</h2>
        <p className="text-[13px] text-[#6b7280] leading-relaxed max-w-[300px]">
          Te enviamos un enlace de activación a <strong className="text-[#111827]">{email}</strong>. Revisa también tu
          carpeta de spam.
        </p>
      </div>
      <div className="w-full flex items-start gap-2.5 px-3.5 py-3 rounded-[14px] text-left" style={{ background: '#f0fdf4', border: '1px solid #dcfce7' }}>
        <span
          className="material-symbols-outlined flex-shrink-0 mt-0.5"
          style={{ fontSize: 16, color: '#16a34a', fontVariationSettings: "'FILL' 1" }}
        >
          check_circle
        </span>
        <p className="text-[12px] font-[700] text-[#15803d]">Tu cuenta fue creada. Actívala con el correo para empezar a adoptar.</p>
      </div>
      <Link
        href="/login"
...
function RefugioSuccessScreen() {
  return (
    <div className="flex flex-col items-center text-center gap-4 py-4">
      <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: '#fef3c7', border: '3px solid #fde68a' }}>
        <span
          className="material-symbols-outlined"
          style={{
            fontSize: 40,
      ...
      <div>
        <h2 className="text-[22px] font-[950] text-[#111827] tracking-tight mb-2">Solicitud enviada</h2>
        <p className="text-[13px] text-[#6b7280] leading-relaxed max-w-[300px]">
      ...
      <div className="w-full flex flex-col gap-2.5">
        {[
          { icon: 'check_circle', color: '#16a34a', bg: '#f0fdf4', border: '#dcfce7', text: 'Solicitud recibida', done: true },
          { icon: 'manage_search', color: '#b45309', bg: '#fffbeb', border: '#fde68a', text: 'Revisión del equipo (1–3 días)', done: false },
          { icon: 'verified', color: '#a1a1aa', bg: '#f9fafb', border: '#e5e7eb', text: 'Aprobación y activación', done: false },
        ].map((item, i) => (
          <div key={i} className="flex items-center gap-3 px-3.5 py-2.5 rounded-[12px]" style={{ background: item.bg, border: `1px solid ${item.border}` }}>
            <span
            ...
            <span className="text-[12px] font-[800]" style={{ color: item.done ? '#15803d' : item.color }}>
              {item.text}
            </span>
          </div>
        ))}
  ...
  if (success && role === 'adoptante') {
    return (
      <AuthLayout
        ...
        rightList={['Filtra por tamaño, energía y más', 'Guarda tus favoritos', 'Proceso de adopción responsable']}
  ...
  if (success && role === 'refugio') {
    return (
      <AuthLayout
        ...
        rightList={['Revisión por nuestro equipo', 'Notificación por correo al aprobarse', 'Acceso completo al portal de refugios']}
        rightActions={[{ label: 'Ver refugios', icon: 'home_work', href: '/refugios' }]}
      >
        <AuthCard>
          <div className="auth-header">
            <AuthBrand />
  ...
  return (
    ...
    >
      <AuthCard>
        ...
        {/* Google + divider igual que LoginView (solo paso 1) */}
        {step === 1 && (
          <>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {ROLES.map(r => (
              ...
              >
                <span className="material-symbols-outlined text-[26px] leading-none" style={{ color: role === r.id ? '#ff6b6b' : '#a1a1aa' }}>
                  {r.icon}
                </span>
                <span className="font-[950] text-[14px] text-[#111827]">{r.label}</span>
                <span className={`font-[700] text-[12px] ${role === r.id ? 'text-[#ff6b6b]/75' : 'text-[#9ca3af]'}`}>
                  {r.desc}
                </span>
              </button>
            ))}
        ...
        {/* ── Formulario por paso ── */}
        <form className="flex flex-col gap-3.5" onSubmit={step < 3 ? handleNext : handleSubmit} noValidate>
          {/* ── PASO 1: Datos básicos ── */}
          {step === 1 && (
            <>
              <p className="text-[11px] font-[950] tracking-[0.18em] uppercase text-[#9ca3af]">Información personal</p>
          ...
          {step === 2 && (
            <>
              <p className="text-[11px] font-[950] tracking-[0.18em] uppercase text-[#9ca3af]">Elige una contraseña segura</p>
              ...
              {data.password && (
                <div className="flex flex-col gap-1.5 -mt-1">
                  ...
                  <span className="text-[12px] font-[900]" style={{ color: passwordStrength.color }}>
          ...
          {step === 3 && role === 'adoptante' && (
            <>
              <p className="text-[11px] font-[950] tracking-[0.18em] uppercase text-[#9ca3af]">Tu dirección</p>
          ...
          {step === 3 && role === 'refugio' && (
            <>
              <p className="text-[11px] font-[950] tracking-[0.18em] uppercase text-[#9ca3af]">Datos del refugio</p>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[13px] font-[900] text-[#1f2937]">
                  Alcaldía donde se ubica <span className="text-[#ff6b6b]">*</span>
                </label>
                <AuthSelect
                  id="ref-alcaldia"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Input
                  ...
                  onChange={e => update('refCapacidad', e.target.value.replace(/\D/g, ''))}
                  leftIcon={<MIcon name="pets" />}
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label htmlFor="ref-desc" className="text-[13px] font-[900] text-[#1f2937]">
              ...
              <div className="flex items-start gap-2.5 px-3 py-3 rounded-[14px]" style={{ background: '#eff6ff', border: '1px solid #dbeafe' }}>
                <span className="material-symbols-outlined flex-shrink-0" style={{ fontSize: 15, color: '#1d4ed8', marginTop: 1 }}>
                  photo_camera
                </span>
                <span className="text-[12px] font-[700] text-[#1d4ed8]">
                  Logo e imágenes podrás subirlas desde tu perfil una vez aprobado.
                </span>
              </div>
