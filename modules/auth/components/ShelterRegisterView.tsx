...
function AuthSelect({
  id, value, onChange, icon, options, placeholder,
}: {
  id: string; value: string; onChange: (v: string) => void
  icon: string; options: string[]; placeholder: string
}) {
  return (
    <div className="relative flex items-center">
      <span className="absolute left-3 pointer-events-none text-[#a1a1aa] material-symbols-outlined" style={{ fontSize: 18 }}>
        {icon}
      </span>
      <select
        id={id}
  ...
  return (
    <div className="flex justify-between items-center relative mb-5">
      {MINI_STEPS.map((label, i) => {
        ...
        return (
          <div key={label} className="relative flex flex-col items-center flex-1 z-[1]">
            {!isLast && (
              <div
            ...
            <div className={[
              ...
              isDone   ? 'border-[#ff6b6b] bg-[#ff6b6b] text-white' :
              isActive ? 'border-[#ff6b6b] text-[#ff6b6b] shadow-[0_0_0_4px_rgba(255,107,107,0.14)]' :
            ...
            <span className={[
              ...
              isActive ? 'text-[#ff6b6b]' : isDone ? 'text-[#3f3f46]' : 'text-[#9ca3af]',
            ].join(' ')}>
              {label}
            </span>
          </div>
...
function FileUploadField({
  label, hint, accept, icon,
}: {
  label: string; hint: string; accept: string; icon: string
}) {
    ...
    <div className="flex flex-col gap-1.5">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-[13px] font-[800] truncate ${fileName ? 'text-[#ff6b6b]' : 'text-[#6b7280]'}`}>
...
function SuccessScreen() {
  const timeline = [
    {
      icon: 'check_circle', color: '#16a34a', bg: '#f0fdf4', border: '#dcfce7',
      label: 'Solicitud recibida', note: 'Tu información fue enviada correctamente.', done: true,
    },
    {
      icon: 'manage_search', color: '#b45309', bg: '#fffbeb', border: '#fde68a',
      label: 'Revisión del equipo', note: 'El equipo verificará la información en 1–3 días.', done: false,
    },
    {
      icon: 'verified', color: '#a1a1aa', bg: '#f9fafb', border: '#e5e7eb',
      label: 'Aprobación y activación', note: 'Recibirás un correo al aprobarse.', done: false,
    },
  ]
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col items-center text-center py-2">
        ...
        <p className="text-[13px] text-[#6b7280] leading-relaxed max-w-[300px]">
          Tu refugio está en revisión. Te notificaremos por correo cuando sea aprobado.
        </p>
      </div>
...
export default function ShelterRegisterView() {
  ...
  const { step, data, update, loading, error, success,
          passwordStrength, handleNext, handleBack, handleSubmit } = reg
    ...
    >
      <AuthCard>
        ...
        <p className="text-[13px] text-[#6b7280] mb-4">
          ¿Eres adoptante?{' '}
          <Link href="/registro" className="text-[#ff6b6b] font-[900] hover:underline">
            Regístrate aquí
          </Link>
        </p>
        ...
        {/* Google — solo en paso 1 */}
        {step === 1 && (
          <>
            <button
              type="button"
              className="w-full flex items-center justify-center gap-2.5 px-3.5 py-2.5 rounded-full
                         border border-[#e5e7eb] bg-white text-[#1f2937] text-[13px] font-[800]
                         hover:bg-[#fafafa] hover:-translate-y-px transition-all duration-150 mb-4"
            >
              <GoogleIcon />
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[11px] font-[900] text-[#9ca3af] uppercase tracking-[0.18em]">o con correo</span>
              <div className="flex-1 h-px bg-[#e5e7eb]" />
            </div>
          </>
        )}
        ...
        {step === 1 && (
          <form className="flex flex-col gap-3.5" onSubmit={handleNext} noValidate>
        ...
        {step === 3 && (
          <form className="flex flex-col gap-3.5" onSubmit={handleSubmit} noValidate>
            ...
            {/* Descripción */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="sr-refdesc" className="text-[13px] font-[900] text-[#1f2937]">
